...
mcp = FastMCP("AutoIR_MCP", instructions="""
    ...
    - 用户提到 “后门”、“计划任务”、“LD_PRELOAD” → 调用 `BackdoorAnalysis`
    - 用户提到 “用户”、“sudo”、“shadow” → 调用 `UserAnalysis`
    - 用户提到 “挖矿”、“异常进程”、“命令执行” → 调用 `ProcAnalysis`
    - 用户提到 “网络连接”、“外联”、“hosts” → 调用 `NetworkAnalysis`
    - 用户提到 “文件”、“webshell”、“/tmp” → 调用 `FileAnalysis`
    - 用户提到 “日志”、“登录记录”、“访问日志” → 调用 `LogAnalysis`
    - 用户提到 “rootkit”、“内核劫持” → 调用 `Rookit`
    
    ## 决策逻辑（必须遵守）
    若用户未指定模块，自行决策，优先对用户提出的内容进行问题分解，调用最可能需要的工具，如果存在可疑内容，但没分析出需要的结果时，应当检查是否有工具最有能进行进一步分析，进行多工具调用（禁止重复调用一个工具两次）。如果分析出对应问题的结果，则停止调用，给出分析结果。
...
def check_hijack():
    ...
    调用说明：
        - 建议在 SSH 连接建立后优先执行。
        - 若检测到劫持且后续计划执行多模块分析，应提示用户是否继续，以防命令错乱或系统卡顿。
...
def check_history():
    """
    bash_history 文件存在性排查工具（UserAnalysis 模块）
...
def check_passwd():
    """
    /etc/passwd 用户权限排查工具（UserAnalysis 模块）
...
def check_ssh_keys():
    """
    SSH authorized_keys 排查工具（UserAnalysis 模块）
...
def check_shadow():
    """
    /etc/shadow 空口令账户排查工具（UserAnalysis 模块）
...
def check_mine():
    ...
    调用说明：
        - 建议在 SSH 连接建立后执行，并先调用 get_ps 获取最新进程列表；
...
def check_exec():
    ...
    调用说明：
        - 在 SSH 连接建立后执行，并先调用 get_ps 获取最新进程列表；
...
def check_exe():
    ...
    调用说明：
        - 建议在 SSH 连接建立后执行，并先调用 get_ps 获取进程快照；
...
def check_network():
    ...
    调用说明：
        - 建议在 SSH 连接建立后执行，并先调用 get_localhost 获取本地 IP；
...
def check_hosts():
    """
    hosts 文件排查工具（NetworkAnalysis 模块）
...
def check_webshell(path='/var/www/html'):
    """
    webroot WebShell 扫描工具（FileAnalysis 模块）
...
def check_ld_so_preload():
    """
    /etc/ld.so.preload 后门排查工具（BackdoorAnalysis 模块）
...
def check_cron():
    """
    计划任务后门排查工具（BackdoorAnalysis 模块）
...
def check_ssh():
    """
    /usr/sbin/sshd 软连接/劫持排查工具（BackdoorAnalysis 模块）
...
def check_ssh_wrapper():
    """
    /usr/sbin/sshd wrapper 后门排查工具（BackdoorAnalysis 模块）
...
def check_inetd():
    """
    /etc/inetd.conf 后门排查工具（BackdoorAnalysis 模块）
...
def check_setuid():
    """
    SUID 后门排查工具（BackdoorAnalysis 模块）
...
def check_profile():
    """
    /etc/profile.d 后门排查工具（BackdoorAnalysis 模块）
...
def check_rc():
    """
    Shell 初始化脚本排查（BackdoorAnalysis 模块）
...
def check_log(path='/var/log/apache2/access.log'):
    """
    Apache2 日志分析工具（FileAnalysis 模块）
